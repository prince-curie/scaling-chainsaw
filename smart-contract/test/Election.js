...
describe("Election", function () {
  it("Should Deploy Election", async function () {
   [chairman, teacher, director,student, student2, student3, notAStakeholder] = await ethers.getSigners()
    ...
    electionfactory.createElection("HeadBoy", ["Godwill", "Prince"])
    console.log(electionfactory.address)
    ethers.getContractAtFromArtifact(Election.json, )
    Election = await ethers.getContractFactory("Election");
    election = await Election.deploy(chairman.address, "Head Boy", 2, ["GoodWill", "Prince"], 1, electionfactory.address);
  ...
  describe("vote()", function () {
    ...
    //   await election.connect(chairman).enableVoting();
